...
def affine_forward(x, w, b):
    ...
    ### MY SOLUTION ###
    #An affine layer follows the form 
    # x @ w + b
    # The shape of x is (2,4,5,6)
    # As pixels are separated into different channels
    # it means that we have to flatten the 2 photos
    # and make x a shape like (2,120) to 
    # make the dot product with w (120,3)
    # it is explained better here https://cs231n.github.io/linear-classify/
    
    #print('x shape', x.shape, type(x))
    #print('wT shape', w.shape, type(w))
    #print('b shape', b.shape,type(b))
    #pixelsFlatten = x[1:].flatten().shape[0]
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        #Check paper
        #Check http://d2l.ai/chapter_convolutional-modern/batch-norm.html
        ...
        #Calculate running mean and running var to use it later
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward_alt(dout, cache):
    ...
    #dL/dx
    dx = 1.0/N * sample_std_inv * (N*dl_xnorm - np.sum(dl_xnorm, axis=0) - x_norm*np.sum(dl_xnorm*x_norm, axis=0))
